...
public:
    ...
    void onButtonClick()
    {
        ...
        {
        case 1:
            DEBUG_LOG("Action: Load Fox Scene");
            loadFoxScene();
            break;
        case 2:
...
private:
    void loadFoxScene()
    {
        DEBUG_LOG("Loading Fox Scene...");
        DEBUG_LOG("Get ready to control the fox!");
        // 不要立即切换场景，而是标记请求
        sceneChangeRequested = true;
        requestedScene = 1; // 1 = FoxScene
    }
...
public:
    ...
    void onUpdate() override
    {
        ...
        if (TouchButton::sceneChangeRequested)
        {
            ...
            if (TouchButton::requestedScene == 1)
            {
                Engine::getInstance().loadScene(std::unique_ptr<Switch2D::Scene>(new FoxScene()));
                return; // 立即返回，不再执行后续代码
            }
            else if (TouchButton::requestedScene == 2)
            {
...
int main(int argc, char *argv[])
{
    ...
    printf("  🔵 Fox Scene - Control the fox! 🦊\n");
    ...
    // 加载场景
    engine.loadScene(std::unique_ptr<Switch2D::Scene>(new DemoScene()));
